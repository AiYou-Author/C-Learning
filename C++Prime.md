...
ATT：创建派生类对象，首先调用基类构造函数，然后再调用派生类构造函数。可以使用初始化列表使用基类构造函数，否则将使用默认的基类构造函数。
...
- 重新定义将隐藏方法
  ...
  如果重新定义继承的方法，应确保与原来的原型完全相同。但如果返回的类型是基类引用或指针，则可以修改指向派生类的引用和指针。此特性称为返回类型协变
...
从baseDMA派生出lackDMA类，而后者不使用new，不需要为lacDMA类定义显示析构函数，复制构造函数和赋值运算符。
